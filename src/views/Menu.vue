<template>
  <v-main>
    ...
    <v-card class="mx-auto" max-width="550">
      ...
      <v-expand-transition>
        <div v-show="show" v-for="profile, index in shopProfileImage" :key="index+0.0029">
          <v-divider></v-divider>

          <v-card-text>
            <div>
              <strong>Owner Name:</strong> {{ profile.owner_name }}
            </div>
            <div>
              <strong>Contact:</strong> {{ profile.phone }}
            </div>
            <div>
              <strong>Tags:</strong> {{ profile.shop_type }}
            </div>
            <div>
...
export default {
  ...
  methods: {
    ...
    getShopProfileImage(){
      ...
      .then(res => {
        this.shopProfileImage = res.data.filter(item => item.slug === this.$store.state.restaurant);
